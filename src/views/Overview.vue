...
export default {
  ...
  mounted() {
    if (this.$store.state.keepNoteOrReview == 'review') {
      this.notes = this.$store.state.randomFive;
    } else {
      console.log('keep note', this.$store.state.collectdata)
    ...
    async showCollectData() {
      try {
        ...
        for (let i = 0; i < response.data.length; i++) {
          if(this.$store.state.collectdata.includes(response.data[i].id)) {
